...
public class FavouriteDBTask {
    ...
    public static FavListBean getFavouriteMsgList(String accountId, int total) {
        ...
        String sql = "select * from " + FavouriteTable.FavouriteDataTable.TABLE_NAME + " where "
                ...
                + accountId + " order by " + FavouriteTable.FavouriteDataTable.MBLOGID + " desc limit " + limit;
        Cursor c = getRsd().rawQuery(sql, null);
        Gson gson = new Gson();
        while (c.moveToNext()) {
    ...
    public static int getTotalNumber(String accountId){
        String searchCount = "select count(" + FavouriteTable.FavouriteDataTable.ID + ") as total"
                + " from " + FavouriteTable.FavouriteDataTable.TABLE_NAME + " where "
                + FavouriteTable.FavouriteDataTable.ACCOUNTID
                + " = " + accountId;
        int total = 0;
