...
public class CommentFragment extends AbstractAppFragment implements
        SwipeRefreshLayout.OnRefreshListener, LoadListView.IXListViewListener {

    private View view;

    private static final String ARGUMENTS_ACCOUNT_EXTRA = CommentFragment.class
            .getName() + ":account_extra";
    private static final String ARGUMENTS_USER_EXTRA = CommentFragment.class
            .getName() + ":userBean_extra";
    private static final String ARGUMENTS_TOKEN_EXTRA = CommentFragment.class
            .getName() + ":token_extra";
            ...
            public void handleMessage(Message msg) {
                ...
                switch (msg.what) {
                    case FIRST_LOAD_TIMELINE:
                        ...
                        adapter = new MentionCommentAdapter(CommentFragment.this, mListView,
                                msgBean, getActivity());
    ...
    public static CommentFragment newInstance(AccountBean accountBean,
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        if (SettingUtility.firstOpenCommentToMe() || total == 0) {
    ...
    private void getNewStatusTask() {
        new Thread() {
            public void run() {
                CommentToMeDao dao = new CommentToMeDao(
                        accountBean.getAccess_token());
                try {
                    CommentListBean newlist = dao.getGSONMsgList();
                    CommentToMeTimeLineDBTask.asyncReplace(newlist, accountBean
                            .getInfo().getId());
    ...
    private void readStatusFromDBTask() {
        new Thread() {
            public void run() {
                int total = CommentToMeTimeLineDBTask
                        .getTotalNumber(accountBean.getUid());
                CommentListBean newlist = CommentToMeTimeLineDBTask
