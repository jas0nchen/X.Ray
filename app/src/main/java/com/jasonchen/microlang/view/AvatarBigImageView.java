 ...
 * 2015/04/10
 */
public class AvatarBigImageView extends AvaterBaseImageView implements ISimRayDrawable {
    ...
    public AvatarBigImageView(Context context) {
        this(context, null);
    }

    public AvatarBigImageView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public AvatarBigImageView(Context context, AttributeSet attrs, int defStyle) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        switch (vType) {
            case UserBean.V_TYPE_PERSONAL:
            	...
            	bitmap =  BitmapFactory.decodeResource(getResources(), R.drawable.avatar_vip_l, opts);
                canvas.drawBitmap(bitmap, getWidth() - bitmap.getWidth(),
                        getHeight() - bitmap.getHeight(), paint);
                break;
            case UserBean.V_TYPE_ENTERPRISE:
                bitmap = BitmapFactory
                        .decodeResource(getResources(), R.drawable.avatar_enterprise_vip_l, opts);
                canvas.drawBitmap(bitmap, getWidth() - bitmap.getWidth(),
                        getHeight() - bitmap.getHeight(), paint);
                break;
            default:
